...
// Package transport provides an http.RoundTripper implementation that routes
...
//go:wasmimport git:bridge/http-bridge@0.1.0 request
...
//go:wasmimport git:bridge/http-bridge@0.1.0 response-status
...
//
//go:wasmimport git:bridge/http-bridge@0.1.0 response-headers
...
//
//go:wasmimport git:bridge/http-bridge@0.1.0 response-body-read
func hostResponseBodyReadRaw(handle uint32, maxBytes uint32, retptr unsafe.Pointer)

//go:wasmimport git:bridge/http-bridge@0.1.0 response-close
...
func (t *Transport) RoundTrip(req *http.Request) (*http.Response, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("transport: failed to marshal headers: %w", err)
	}
	...
	// Call host function
	mPtr, mLen := ptrAndLen(methodBytes)
	uPtr, uLen := ptrAndLen(urlBytes)
	hPtr, hLen := ptrAndLen(headersJSON)
	bPtr, bLen := ptrAndLen(bodyBytes)
	handle := hostHTTPRequest(mPtr, mLen, uPtr, uLen, hPtr, hLen, bPtr, bLen)
