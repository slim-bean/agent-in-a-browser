...
// On wasm32-wasi, std::sync::Mutex panics on recursive acquisition
// (no_threads mode). The wake_by_ref fix below (collect-then-wake)
// prevents re-entrancy, so std::sync::Mutex is safe to use.
#[cfg(feature = "std")]
    ...
    Fut::Output: Clone,
{
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        ...
        // Collect wakers while holding the lock, then wake them after
        // releasing it. Prevents recursive mutex acquisition on wasm32-wasi.
        let wakers_to_wake: alloc::vec::Vec<Waker> = {
            #[cfg(feature = "std")]
            ...
            let collected: alloc::vec::Vec<Waker> = wakers.drain().flatten().collect();
            drop(wakers_guard);
            collected
        };
...
impl ArcWake for Notifier {
    fn wake_by_ref(arc_self: &Arc<Self>) {
        // Collect wakers while holding the lock, then wake them after
        // releasing it. This prevents recursive mutex acquisition on
        // wasm32-wasi where std::sync::Mutex panics on re-entrancy.
        // A woken task may immediately poll the same Shared future,
        // which would try to acquire this same lock.
        let wakers_to_wake: alloc::vec::Vec<Waker> = {
            #[cfg(feature = "std")]
            ...
            if let Some(wakers) = wakers.as_mut() {
                wakers.iter_mut().filter_map(|(_key, opt)| opt.take()).collect()
            } else {
                alloc::vec::Vec::new()
            }
        }; // lock dropped here
